...
def test_pid_multidimensional():
	...
	values = run_pid(pid, 0, 10, 0.1, currentValue=currentValue, desiredValue=desiredValue)

	assert np.array_equal(np.round(np.sum(values, axis=0), 2), np.array([100.99, 400.64, 107.36]))
